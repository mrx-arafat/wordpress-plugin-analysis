...
class Xpo {
	...
	public static function get_lc_key() {
		return get_option( 'edd_revenue_license_key', '' );
	}
	...
	public static function get_option_without_cache( $option, $default_value = false ) {
		...
		if ( is_object( $row ) ) {
			...
			return apply_filters( "revx_default_option_{$option}", $default_value, $option );
		}

		return apply_filters( "revx_option_{$option}", maybe_unserialize( $value ), $option );
	}
	...
	public static function add_option_without_cache( $option, $value = '', $autoload = 'yes' ) {
		...
		*/
		...
		if ( apply_filters( "revx_default_option_{$option}", false, $option, false ) !== self::get_option_without_cache( $option ) ) {
			return false;
		}
	...
	public static function get_transient_without_cache( $transient ) {
		...
		return apply_filters( "revx_transient_{$transient}", $value, $transient );
	}
	...
	public static function generate_utm_link( $params ) {
		...
		$base_url      = $params['url'] ?? 'https://www.wowrevenue.com/';
		$utm_key       = $params['utmKey'] ?? null;
		$affiliate     = $params['affiliate'] ?? apply_filters( 'revx_affiliate_id', '' );
