...
$buider_data      = revenue()->get_campaign_meta( $campaign['id'], 'builderdata', true )['drawer'];
$generated_styles = revenue()->campaign_style_generator( 'drawer', $campaign );
...
$reward_message  = ''; // Default message
...
foreach ( $offers as $index => $offer ) {
	...
	if ( 0 == $cart_total ) {
		...
		$remaining_amount = $cart_total - $offer['required_goal'];
...
$has_upsell_products = ! empty( $upsell_products ) && 'yes' === $campaign['upsell_products_status'];
...
<div id="revx-progress-drawer"
	class="revx-campaign-drawer revx-campaign-fsb <?php echo esc_attr( $position ); ?> <?php echo esc_attr( $has_upsell_products ? 'has-upsell' : 'no-upsell' ); ?> <?php echo esc_attr( $cart_total <= 0 ? 'hide' : '' ); ?>"
	...
	<div class="revx-drawer-content <?php echo $has_upsell_products ? 'revx-has-upsell' : ''; ?>"  style="<?php echo esc_attr( $container_style ); ?>">
		<!-- Close Button -->
		<button class="revx-close-btn">
			<?php echo revenue()->get_svg_from_assets( 'cross' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		</button>

		<div class="revx-circular-progress-container " style="<?php echo esc_attr( $circular_progress_container_style ); ?>">
			<!-- Circular Progress -->
			<div class="revx-circular-progress">
				<?php
				if ( $has_upsell_products ) {
					?>
						<svg style="<?php echo esc_attr( $circular_progress_bar_style ); ?>" height="<?php echo esc_attr( $circular_size ); ?>" width="<?php echo esc_attr( $circular_size ); ?>">
							<circle
								class="revx-progress-empty"
								stroke="<?php echo esc_attr( 'var(--revx-empty-color, #31353F)' ); ?>"
							...
							<circle
								...
								stroke="<?php echo esc_attr( 'var(--revx-filled-color, #31353F)' ); ?>"
								fill="none"
				...
				if ( 'yes' === $is_show_progress_bar ) {
					?>
						<div class="revx-circular-text"><?php echo esc_html( round( $progress ) ); ?>%</div>
					<?php
				} else {
					?>
						<div class="revx-circular-progress-icon"><?php echo revenue()->get_svg_from_assets( 'free_shipping' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
					<?php
				}
				?>
...
?>
								...
								?>
									<div class="revx-upsell-slider-product-card">
										...
										<div class="revx-upsell-slider-product-details">
											...
											<div class="revx-upsell-slider-actions revx-align-center">
	...
	<input type="hidden" name="revenue_free_shipping_offer" value="<?php echo esc_html( htmlspecialchars( wp_json_encode( $offers ) ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" />
	<input type="hidden" name="revenue_upsell_products" value="<?php echo esc_html( htmlspecialchars( wp_json_encode( $upsell_products ) ) ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" />
</div>
