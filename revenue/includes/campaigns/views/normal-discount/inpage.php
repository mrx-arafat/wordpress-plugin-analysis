		...
		foreach ( $offered_product_ids as $product_index => $offer_product_id ) {
			...
			<div class="<?php echo esc_attr( $container_class ); ?>" data-product-id="<?php echo esc_attr( ! $offered_product->is_type( 'variable' ) ? $offer_product_id : '' ); ?>" style="<?php echo esc_attr( $is_tag_enabled ? $tagged_product_style : $product_style ); ?>">
				...
				if ( 'list' == $view_mode ) {
					...
					?>
					<div class="revx-campaign-text-content revx-justify-space-sm revx-full-width" style="gap: 8px;">
			...
			<?php
			if ( ( 1 == $total_offer_products && 'list' === $view_mode && $is_last_product ) ) {
				?>
				<div class="revx-last-add-button">
					<?php
						echo wp_kses(
							revenue()->get_template_part(
								'add_to_cart',
...
if ( 'list' == $view_mode ) {
	...
	?>
		<div class="revx-campaign-container__wrapper revx-slider-items-wrapper" style="<?php echo esc_attr( $wrapper_style ); ?>">
