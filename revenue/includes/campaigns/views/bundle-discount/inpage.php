 ...
 *
 * This file handles the display of bundle discount offers in a inpage container.
 *
 * @package    Revenue
...
if ( is_array( $trigger_items ) ) {
	...
	foreach ( $trigger_items as $offer_index => $offer ) {
		...
		if ( $offer_product_id == $product->get_id() && isset( $offer['trigger'] ) && $offer['trigger'] ) {
			$offer_qty     = 1;
...
if ( 'list' == $view_mode ) {
	...
	<div class="revx-campaign-container__wrapper revx-slider-items-wrapper revx-slider-icon-parent" style="<?php echo esc_attr( $wrapper_style ); ?>" data-bundle_products="<?php echo esc_html( htmlspecialchars( wp_json_encode( $trigger_items ) ) ); ?>">
		...
		<div class="revx-total-price" style="<?php echo esc_attr( $total_price_style ); ?>">
			<?php
			echo wp_kses( revenue()->tag_wrapper( $campaign, $generated_styles, 'bundleLabel', '', 'revx-builder__popular-tag revx-bundle-offer-label', 'div' ), revenue()->get_allowed_tag() );
			echo wp_kses( revenue()->tag_wrapper( $campaign, $generated_styles, 'totalPriceText', __( 'Total', 'revenue' ), 'revx-total-price-title', 'h4' ), revenue()->get_allowed_tag() );
			?>
			<div class="revx-total-price__offer-price revx-align-center">
				<?php
